...
Global and static variables are zero-initialized by the startup code (covered in previous chapters). Local automatic variables are not initialized unless you explicitly do so. Using an uninitialized local is undefined behavior.
...
Previous chapters introduced `volatile` for registers. It also matters for variables shared with ISRs (also mentioned before). Here are additional subtleties.
...
For flash-resident data, use `const` to place data in `.rodata`:
